...
int main(void) {
    ...
    */
    ...
    Frequency_Detector Detector_200(width, height, 200.0, 30.0, 3);
    ...
    Slicer.doEveryTimeInterval(Rate, [&](const dv::EventStore &Events) {
        ...
        if (!Events.isEmpty()) {
            ...
            Detector_100.Accept_Event_Batch(Events, detectionMask);
            Detector_100.Highlight_Pixels(detectionMask, cv::Vec3b(255, 255, 255));
            
            Detector_200.Accept_Event_Batch(Events, detectionMask);
            Detector_200.Highlight_Pixels(detectionMask, cv::Vec3b(255, 0, 255));
